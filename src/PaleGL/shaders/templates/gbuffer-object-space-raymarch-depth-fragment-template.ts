...
f=uColor*u;
#endif
vec3 m=vWorldPosition,d=m,z=uIsPerspective>.5?normalize(m-uViewPosition):normalize(-uViewPosition);float i=0.,r=0.;vec3 y=d;float a=EPS;mat4 s=vInverseWorldMatrix;for(int l=0;l<OI;l++){y=d+z*r;i=o(y,s,uBoundsScale,uUseWorld).x;r+=i;if(!x(v(y,s,uBoundsScale),uBoundsScale))break;if(i<=a)break;}if(i>a)discard;vec4 l=uProjectionMatrix*uViewMatrix*vec4(y,1);gl_FragDepth=l.z/l.w*.5+.5;float p=f.w;
#ifdef USE_ALPHA_TEST
