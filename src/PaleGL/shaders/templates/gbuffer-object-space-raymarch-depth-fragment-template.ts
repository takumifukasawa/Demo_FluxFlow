...
vec3 v(vec3 v,mat4 u,vec3 f){return(u*vec4(v,1)).xyz*f;}vec2 o(vec3 u,mat4 f,vec3 d,float m){vec3 z=mix(v(u,f,d),u,m);return dfScene(z);}bool n(vec3 v,vec3 u){return abs(v.x)<u.x*.5+1e-4&&abs(v.y)<u.y*.5+1e-4&&abs(v.z)<u.z*.5+1e-4;}void x(float v,float u){if(v<u)discard;}uniform vec4 uColor;uniform sampler2D uDiffuseMap;uniform vec2 uDiffuseMapUvScale;uniform float uIsPerspective,uUseWorld;uniform vec3 uBoundsScale;uniform sampler2D uDepthTexture;
...
f=uColor*u;
#endif
vec3 m=vWorldPosition,d=m,z=uIsPerspective>.5?normalize(m-uViewPosition):normalize(-uViewPosition);float i=0.,r=0.;vec3 y=d;float s=EPS;mat4 l=vInverseWorldMatrix;for(int U=0;U<OI;U++){y=d+z*r;i=o(y,l,uBoundsScale,uUseWorld).x;r+=i;if(!n(v(y,l,uBoundsScale),uBoundsScale))break;if(i<=s)break;}if(i>s)discard;vec4 U=uProjectionMatrix*uViewMatrix*vec4(y,1);gl_FragDepth=U.z/U.w*.5+.5;float p=f.w;
#ifdef USE_ALPHA_TEST
