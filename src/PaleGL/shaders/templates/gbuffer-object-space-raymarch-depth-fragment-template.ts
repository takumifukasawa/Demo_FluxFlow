...
f=uColor*u;
#endif
vec3 d=vWorldPosition,m=uIsPerspective>.5?normalize(vWorldPosition-uViewPosition):normalize(-uViewPosition);float o=0.,r=0.;vec3 i=d;for(int y=0;y<64;y++){i=d+m*r;o=dfScene(v(i,vInverseWorldMatrix,uBoundsScale)).x;r+=o;if(!n(v(i,vInverseWorldMatrix,uBoundsScale),uBoundsScale)||o<=1e-4)break;}if(o>1e-4)discard;vec4 s=uProjectionMatrix*uViewMatrix*vec4(i,1);gl_FragDepth=s.z/s.w*.5+.5;float y=f.w;
#ifdef USE_ALPHA_TEST
