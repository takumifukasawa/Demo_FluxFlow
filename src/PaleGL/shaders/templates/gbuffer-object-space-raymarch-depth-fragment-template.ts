...
v=uColor*u;
#endif
vec3 f=vWorldPosition,z=uIsPerspective>.5?normalize(vWorldPosition-uViewPosition):normalize(-uViewPosition);float m=0.,r=0.;vec3 l=f;for(int h=0;h<64;h++){l=f+z*r;m=dfScene(o(l,vInverseWorldMatrix,uBoundsScale)).x;r+=m;if(!p(o(l,vInverseWorldMatrix,uBoundsScale),uBoundsScale)||m<=1e-4)break;}if(m>1e-4)discard;vec4 s=uProjectionMatrix*uViewMatrix*vec4(l,1);gl_FragDepth=s.z/s.w*.5+.5;float h=v.w;
#ifdef USE_ALPHA_TEST
