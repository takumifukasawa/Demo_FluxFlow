...
vec3 v(vec3 v,mat4 u,vec3 d){return(u*vec4(v,1)).xyz*d;}vec2 o(vec3 u,mat4 f,vec3 d,float m){vec3 y=mix(v(u,f,d),u,m);return dfScene(y);}bool n(vec3 v,vec3 u){return abs(v.x)<u.x*.5+1e-4&&abs(v.y)<u.y*.5+1e-4&&abs(v.z)<u.z*.5+1e-4;}void x(float v,float u){if(v<u)discard;}uniform vec4 uColor;uniform sampler2D uDiffuseMap;uniform vec2 uDiffuseMapUvScale;uniform float uIsPerspective,uUseWorld;uniform vec3 uBoundsScale;uniform sampler2D uDepthTexture;
...
f=uColor*u;
#endif
vec3 m=vWorldPosition,d=m,y=uIsPerspective>.5?normalize(m-uViewPosition):normalize(-uViewPosition);float i=0.,r=0.;vec3 s=d;float z=EPS;mat4 P=vInverseWorldMatrix;for(int l=0;l<OI;l++){s=d+y*r;i=o(s,P,uBoundsScale,uUseWorld).x;r+=i;if(!n(v(s,P,uBoundsScale),uBoundsScale))break;if(i<=z)break;}if(i>z)discard;vec4 a=uProjectionMatrix*uViewMatrix*vec4(s,1);gl_FragDepth=a.z/a.w*.5+.5;float l=f.w;
#ifdef USE_ALPHA_TEST
