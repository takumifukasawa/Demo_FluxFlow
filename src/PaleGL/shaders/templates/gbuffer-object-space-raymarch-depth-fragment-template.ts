...
#define PI 3.14
#define PI2 6.28
vec2 u(vec2 u,vec2 v){return u.x<v.x?u:v;}mat2 u(float u){float v=sin(u),f=cos(u);return mat2(f,v,-v,f);}vec2 v(vec2 v,float o){return v*u(-o);}vec3 s(vec3 u,vec3 v){return u-v;}vec3 n(vec3 u,vec3 v){return u/v;}float o(float u,vec3 v){return u*min(v.x,min(v.y,v.z));}vec2 n(vec2 u){float f=PI/10.-atan(u.x,u.y),x=PI*2./10.;f=floor(f/x)*x;return v(u,-f);}float opSm(float u,float v,float x){float f=clamp(.5+.5*(v-u)/x,0.,1.);return mix(v,u,f)-x*f*(1.-f);}float dfSp(vec3 u,float v){return length(u)-v;}float dfTo(vec3 v,vec2 u){return length(vec2(length(v.xz)-u.x,v.y))-u.y;}float n(vec3 u,float v,float o){vec2 f=vec2(length(u.xz)-2.*v+.05,abs(u.y)-o);return min(max(f.x,f.y),0.)+length(max(f,0.))-.05;}float opWing(vec3 u,vec3 f,float x,vec2 m){u=s(u,vec3(m.xy,0));u.xy=v(u.xy,x);u.yz=v(u.yz,PI*.5);u=n(u,f);float z=n(u,1.,.1);return o(z,f);}vec2 opButterfly(vec3 u,float o){u/=2.;vec3 f=u;f.yz=v(f.yz,-PI*.5);f.x=abs(f.x);f.xz=v(f.xz,PI*sin(sin(uTimelineTime*10.+o)*cos(uTimelineTime*6.+o))*.3);float m=opWing(f,vec3(.4,.2,.24)*.2,PI*-.3,vec2(.5,.4)*.2),s=opWing(f,vec3(.32,.2,.2)*.2,PI*.3,vec2(.4,-.4)*.2);return vec2(min(m,s),0);}vec2 opFlower(vec3 f){vec2 m=vec2(1e4,-1e4);f.y-=-.8;float z=.1;z=sin(uTimelineTime*2.4+.2)*.5;float x=-.1;x=cos(uTimelineTime*2.6+.1)*-.5;float y=f.y*sin(f.y*z),P=f.y*sin(f.y*x),l=sin(1.12*z)*.28,e=sin(1.12*x)*.28;mat2 d=u(-1.12*z),r=u(1.12*x);vec3 i=f;i=s(i,vec3(y,.28,P));float U=n(i,.015,.28);m=u(m,vec2(U,1));vec3 B=f;B=s(B,vec3(l,.56,e));B.yz=v(B.yz,PI*.5);B.xz*=d;B.yz*=r;B.xy=n(B.xy);B=s(B,vec3(0,.2,sin(B.y*5.)*.105));B.yz=v(B.yz,PI*.5);vec3 C=vec3(.08,.2,.2);B=n(B,C);float h=n(B,.4,.01);h=o(h,C);m=u(m,vec2(h,2));vec3 p=f;p=s(p,vec3(l,.59,e));float D=dfSp(p,.04);return u(m,vec2(D,3));}float opDb(float u,float v,float f,float m,float s,float x){return mix(mix(mix(v,f,smoothstep(0.,.25,u)),mix(f,m,smoothstep(.25,.5,u)),smoothstep(.25,.5,u)),mix(mix(m,s,smoothstep(.5,.75,u)),mix(s,x,smoothstep(.75,1.,u)),smoothstep(.5,.75,u)),smoothstep(.5,1.,u));}float opTb(float u,float v,float f,float x){return mix(mix(v,f,smoothstep(0.,.5,u)),mix(f,x,smoothstep(.5,1.,u)),smoothstep(.5,1.,u));}
#define BN 16
#define FS 1.
#define CS.35
uniform vec3 uCP,uBPs[BN];float dfMB(vec3 u,float v){for(int f=0;f<BN;f++){float m=dfSp(s(u,uBPs[f].xyz),CS);v=opSm(v,m,.25);}float f=sin(u.x*4.+uTimelineTime*3.4)*.07+cos(u.y*3.+uTimelineTime*3.2)*.07+sin(u.z*3.5+uTimelineTime*3.)*.07;v+=f*(1.-smoothstep(1.,1.8,length(u-uCP)));return v;}vec2 opMo(vec2 u,vec2 v,float f){return mix(u,v,f);}
...
f=uColor*u;
#endif
vec3 v=vWorldPosition,s=uIsPerspective>.5?normalize(vWorldPosition-uViewPosition):normalize(-uViewPosition);float m=0.,r=0.;vec3 B=v;for(int z=0;z<64;z++){B=v+s*r;m=dfScene(o(B,vInverseWorldMatrix,uBoundsScale)).x;r+=m;if(!p(o(B,vInverseWorldMatrix,uBoundsScale),uBoundsScale)||m<=1e-4)break;}if(m>1e-4)discard;vec4 z=uProjectionMatrix*uViewMatrix*vec4(B,1);gl_FragDepth=z.z/z.w*.5+.5;float h=f.w;
#ifdef USE_ALPHA_TEST
