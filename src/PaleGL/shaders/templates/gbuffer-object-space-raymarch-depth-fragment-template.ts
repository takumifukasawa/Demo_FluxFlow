...
v=uColor*u;
#endif
vec3 x=vWorldPosition,y=uIsPerspective>.5?normalize(vWorldPosition-uViewPosition):normalize(-uViewPosition);float l=0.,r=0.;vec3 z=x;for(int i=0;i<64;i++){z=x+y*r;l=dfScene(o(z,vInverseWorldMatrix,uBoundsScale)).x;r+=l;if(!m(o(z,vInverseWorldMatrix,uBoundsScale),uBoundsScale)||l<=1e-4)break;}if(l>1e-4)discard;vec4 s=uProjectionMatrix*uViewMatrix*vec4(z,1);gl_FragDepth=s.z/s.w*.5+.5;float i=v.w;
#ifdef USE_ALPHA_TEST
f(i,uAlphaTestThreshold);
#endif
outColor=vec4(1);}`;