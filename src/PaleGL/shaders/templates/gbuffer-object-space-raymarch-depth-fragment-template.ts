...
f=uColor*v;
#endif
vec3 o=vWorldPosition,m=uIsPerspective>.5?normalize(vWorldPosition-uViewPosition):normalize(-uViewPosition);float i=0.,r=0.;vec3 y=o;for(int l=0;l<80;l++){y=o+m*r;i=d(y,vInverseWorldMatrix,uBoundsScale,uUseWorld).x;r+=i;if(!n(u(y,vInverseWorldMatrix,uBoundsScale),uBoundsScale)||i<=1e-4)break;}if(i>1e-4)discard;vec4 s=uProjectionMatrix*uViewMatrix*vec4(y,1);gl_FragDepth=s.z/s.w*.5+.5;float z=f.w;
#ifdef USE_ALPHA_TEST
x(z,uAlphaTestThreshold);
#endif
outColor=vec4(1);}`;