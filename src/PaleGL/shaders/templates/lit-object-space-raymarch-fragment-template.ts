...
#ifdef USE_NORMAL_MAP
vec3 n(vec3 v,vec3 u,vec3 f,sampler2D x,vec2 o){vec3 m=texture(x,o).xyz;m=m*2.-1.;return normalize(mat3(normalize(u),normalize(f),normalize(v))*m);}
#endif
layout(location=0) out vec4 outGBufferA;layout(location=1) out vec4 outGBufferB;layout(location=2) out vec4 outGBufferC;layout(location=3) out vec4 outGBufferD;void main(){vec4 f=vec4(0,0,0,1);vec2 o=vUv*uDiffuseMapUvScale;vec4 d=uDiffuseColor*texture(uDiffuseMap,o);vec3 m=vNormal;
#ifdef USE_NORMAL_MAP
m=n(vNormal,vTangent,vBinormal,uNormalMap,o);
#else
m=normalize(vNormal);
#endif
...
#endif
vec3 z=vWorldPosition,y=uIsPerspective>.5?normalize(vWorldPosition-uViewPosition):normalize(-uViewPosition);vec2 r=vec2(0);float l=0.;vec3 i=z;for(int D=0;D<100;D++){i=z+y*l;r=v(i,vInverseWorldMatrix,uBoundsScale);l+=r.x;if(!x(u(i,vInverseWorldMatrix,uBoundsScale),uBoundsScale)||r.x<=1e-4)break;}if(r.x>1e-4)discard;float D=texelFetch(uDepthTexture,ivec2(gl_FragCoord.xy),0).x,P=p(D,uNearClip,uFarClip);vec4 G=uViewMatrix*vec4(i,1);float C=s(G.z,uNearClip,uFarClip);if(C>=P)discard;vec4 a=uProjectionMatrix*uViewMatrix*vec4(i,1);gl_FragDepth=a.z/a.w*.5+.5;if(r.x>0.)m=x(i,vInverseWorldMatrix,uBoundsScale);f=d;
#ifdef USE_ALPHA_TEST
p(f.w,uAlphaTestThreshold);
#endif
f.xyz=pow(f.xyz,vec3(2.2));outGBufferA=vec4(f.xyz,1);outGBufferB=s(m,uShadingModelId);outGBufferC=vec4(uMetallic,uRoughness,0,1);outGBufferD=vec4(U.xyz,1);}`;