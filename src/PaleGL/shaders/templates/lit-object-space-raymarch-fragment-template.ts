...
#endif
layout(location=0) out vec4 outGBufferA;layout(location=1) out vec4 outGBufferB;layout(location=2) out vec4 outGBufferC;layout(location=3) out vec4 outGBufferD;void main(){vec4 m=vec4(0,0,0,1);vec2 z=vUv*uDiffuseMapUvScale;vec4 x=uDiffuseColor*texture(uDiffuseMap,z);vec3 i=vNormal;
#ifdef USE_NORMAL_MAP
i=e(vNormal,vTangent,vBinormal,uNormalMap,z);
#else
i=normalize(vNormal);
#endif
...
x*=vVertexColor;
#endif
x=vec4(1);vec3 d=vWorldPosition,y=uIsPerspective>.5?normalize(vWorldPosition-uViewPosition):normalize(-uViewPosition);vec2 l=vec2(0);float r=0.;vec3 U=d;for(int D=0;D<100;D++){U=d+y*r;l=s(U,vInverseWorldMatrix,uBoundsScale);r+=l.x;if(!e(o(U,vInverseWorldMatrix,uBoundsScale),uBoundsScale)||l.x<=1e-4)break;}if(l.x>1e-4)discard;float D=texelFetch(uDepthTexture,ivec2(gl_FragCoord.xy),0).x,n=v(D,uNearClip,uFarClip);vec4 G=uViewMatrix*vec4(U,1);float C=e(G.z,uNearClip,uFarClip);if(C>=n)discard;vec4 P=uProjectionMatrix*uViewMatrix*vec4(U,1);gl_FragDepth=P.z/P.w*.5+.5;if(l.x>0.)i=u(U,vInverseWorldMatrix,uBoundsScale);m=x;
#ifdef USE_ALPHA_TEST
f(m.w,uAlphaTestThreshold);
#endif
m.xyz=pow(m.xyz,vec3(2.2));outGBufferA=vec4(m.xyz,1);outGBufferB=p(i,uShadingModelId);outGBufferC=vec4(uMetallic,uRoughness,0,1);outGBufferD=vec4(uEmissiveColor.xyz.xyz,1);}`;