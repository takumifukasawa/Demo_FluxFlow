...
#define FS 1.
#define CS.35
uniform vec3 uCP,uBPs[BN];float dfMB(vec3 u,float v){for(int f=0;f<BN;f++){float x=dfSp(n(u,uBPs[f].xyz),CS);v=opSm(v,x,.25);}float f=sin(u.x*4.+uTime*3.4)*.1+sin(u.y*3.+uTime*3.2)*.1+sin(u.z*3.5+uTime*3.)*.1;v+=f*(1.-smoothstep(1.,1.8,length(u-uCP)));return v;}vec2 opMo(vec2 u,vec2 v,float f){return mix(u,v,f);}
