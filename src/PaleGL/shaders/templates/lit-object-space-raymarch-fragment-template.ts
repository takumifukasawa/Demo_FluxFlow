...
#endif
vec3 l=vWorldPosition,r=l,y=uIsPerspective>.5?normalize(l-uViewPosition):normalize(-uViewPosition);vec2 U=vec2(0);float e=0.;vec3 p=r;float S=EPS;mat4 P=vInverseWorldMatrix;for(int D=0;D<OI;D++){p=r+y*e;U=o(p,P,uBoundsScale,uUseWorld);e+=U.x;if(!n(v(p,P,uBoundsScale),uBoundsScale))break;if(U.x<=S)break;}if(U.x>S)discard;float D=texelFetch(uDepthTexture,ivec2(gl_FragCoord.xy),0).x,a=d(D,uNearClip,uFarClip);vec4 G=uViewMatrix*vec4(p,1);float O=n(G.z,uNearClip,uFarClip);if(O>=a)discard;vec4 c=uProjectionMatrix*uViewMatrix*vec4(p,1);gl_FragDepth=c.z/c.w*.5+.5;if(U.x>0.)i=u(p,P,uBoundsScale,uUseWorld);f=m;
#ifdef USE_ALPHA_TEST
d(f.w,uAlphaTestThreshold);
#endif
f.xyz=pow(f.xyz,vec3(2.2));float I=uMetallic;I*=texture(uMetallicMap,z*uMetallicMapTiling.xy).x;float C=uRoughness;C*=texture(uRoughnessMap,z*uRoughnessMapTiling.xy).x;outGBufferA=vec4(f.xyz,1);outGBufferB=x(i,uShadingModelId);outGBufferC=vec4(I,C,0,1);outGBufferD=vec4(s.xyz,1);}`;