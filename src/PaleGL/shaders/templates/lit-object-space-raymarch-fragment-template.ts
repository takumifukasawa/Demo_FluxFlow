...
x*=vVertexColor;
#endif
x=vec4(1);vec3 n=vWorldPosition,y=uIsPerspective>.5?normalize(vWorldPosition-uViewPosition):normalize(-uViewPosition);vec2 l=vec2(0);float r=0.;vec3 U=n;for(int D=0;D<100;D++){U=n+y*r;l=s(U,vInverseWorldMatrix,uBoundsScale);r+=l.x;if(!e(o(U,vInverseWorldMatrix,uBoundsScale),uBoundsScale)||l.x<=1e-4)break;}if(l.x>1e-4)discard;float D=texelFetch(uDepthTexture,ivec2(gl_FragCoord.xy),0).x,P=v(D,uNearClip,uFarClip);vec4 G=uViewMatrix*vec4(U,1);float d=e(G.z,uNearClip,uFarClip);if(d>=P)discard;vec4 C=uProjectionMatrix*uViewMatrix*vec4(U,1);gl_FragDepth=C.z/C.w*.5+.5;if(l.x>0.)m=u(U,vInverseWorldMatrix,uBoundsScale);i=x;
#ifdef USE_ALPHA_TEST
