...
#endif
vec3 z=vWorldPosition,x=z,e=uIsPerspective>.5?normalize(z-uViewPosition):normalize(-uViewPosition);vec2 l=vec2(0);float U=0.;vec3 p=x;float S=EPS;mat4 P=vInverseWorldMatrix;for(int D=0;D<OI;D++){p=x+e*U;l=o(p,P,uBoundsScale,uUseWorld);U+=l.x;if(!n(v(p,P,uBoundsScale),uBoundsScale))break;if(l.x<=S)break;}if(l.x>S)discard;float D=texelFetch(uDepthTexture,ivec2(gl_FragCoord.xy),0).x,a=d(D,uNearClip,uFarClip);vec4 G=uViewMatrix*vec4(p,1);float O=n(G.z,uNearClip,uFarClip);if(O>=a)discard;vec4 c=uProjectionMatrix*uViewMatrix*vec4(p,1);gl_FragDepth=c.z/c.w*.5+.5;if(l.x>0.)i=u(p,P,uBoundsScale,uUseWorld);f=y;
#ifdef USE_INSTANCING
...
#endif
f.xyz=pow(f.xyz,vec3(2.2));float I=uMetallic;I*=texture(uMetallicMap,m*uMetallicMapTiling.xy).x;float C=uRoughness;C*=texture(uRoughnessMap,m*uRoughnessMapTiling.xy).x;outGBufferA=vec4(f.xyz,1);outGBufferB=s(i,uShadingModelId);outGBufferC=vec4(I,C,0,1);outGBufferD=vec4(r.xyz,1);}`;