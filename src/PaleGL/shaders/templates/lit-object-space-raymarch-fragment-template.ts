...
#endif
vec3 p=vWorldPosition,x=p,y=uIsPerspective>.5?normalize(p-uViewPosition):normalize(-uViewPosition);vec2 U=vec2(0);float i=0.;vec3 e=x;float S=EPS;mat4 P=vInverseWorldMatrix;for(int D=0;D<OI;D++){e=x+y*i;U=o(e,P,uBoundsScale,uUseWorld);i+=U.x;if(!n(v(e,P,uBoundsScale),uBoundsScale))break;if(U.x<=S)break;}if(U.x>S)discard;float D=texelFetch(uDepthTexture,ivec2(gl_FragCoord.xy),0).x,a=d(D,uNearClip,uFarClip);vec4 G=uViewMatrix*vec4(e,1);float O=n(G.z,uNearClip,uFarClip);if(O>=a)discard;vec4 c=uProjectionMatrix*uViewMatrix*vec4(e,1);gl_FragDepth=c.z/c.w*.5+.5;if(U.x>0.)l=u(e,P,uBoundsScale,uUseWorld);f=z;
#ifdef USE_ALPHA_TEST
d(f.w,uAlphaTestThreshold);
#endif
f.xyz=pow(f.xyz,vec3(2.2));outGBufferA=vec4(f.xyz,1);outGBufferB=s(l,uShadingModelId);outGBufferC=vec4(uMetallic,uRoughness,0,1);outGBufferD=vec4(r.xyz,1);}`;