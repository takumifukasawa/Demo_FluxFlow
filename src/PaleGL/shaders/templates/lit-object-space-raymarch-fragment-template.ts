...
d*=vVertexColor;
#endif
vec3 n=vWorldPosition,y=uIsPerspective>.5?normalize(vWorldPosition-uViewPosition):normalize(-uViewPosition);vec2 r=vec2(0);float l=0.;vec3 U=n;for(int i=0;i<100;i++){U=n+y*l;r=s(U,vInverseWorldMatrix,uBoundsScale);l+=r.x;if(!e(o(U,vInverseWorldMatrix,uBoundsScale),uBoundsScale)||r.x<=1e-4)break;}if(r.x>1e-4)discard;float i=texelFetch(uDepthTexture,ivec2(gl_FragCoord.xy),0).x,P=v(i,uNearClip,uFarClip);vec4 D=uViewMatrix*vec4(U,1);float G=e(D.z,uNearClip,uFarClip);if(G>=P)discard;vec4 C=uProjectionMatrix*uViewMatrix*vec4(U,1);gl_FragDepth=C.z/C.w*.5+.5;if(r.x>0.)z=u(U,vInverseWorldMatrix,uBoundsScale);m=d;
#ifdef USE_ALPHA_TEST
