...
z*=vVertexColor;
#endif
z=vec4(1);vec3 d=vWorldPosition,y=uIsPerspective>.5?normalize(vWorldPosition-uViewPosition):normalize(-uViewPosition);vec2 n=vec2(0);float r=0.;vec3 U=d;for(int i=0;i<100;i++){U=d+y*r;n=s(U,vInverseWorldMatrix,uBoundsScale);r+=n.x;if(!e(o(U,vInverseWorldMatrix,uBoundsScale),uBoundsScale)||n.x<=1e-4)break;}if(n.x>1e-4)discard;float i=texelFetch(uDepthTexture,ivec2(gl_FragCoord.xy),0).x,P=v(i,uNearClip,uFarClip);vec4 D=uViewMatrix*vec4(U,1);float G=e(D.z,uNearClip,uFarClip);if(G>=P)discard;vec4 C=uProjectionMatrix*uViewMatrix*vec4(U,1);gl_FragDepth=C.z/C.w*.5+.5;if(n.x>0.)l=u(U,vInverseWorldMatrix,uBoundsScale);m=z;
#ifdef USE_ALPHA_TEST
