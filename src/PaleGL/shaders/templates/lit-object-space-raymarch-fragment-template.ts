...
x*=vVertexColor;
#endif
vec3 d=vWorldPosition,y=uIsPerspective>.5?normalize(vWorldPosition-uViewPosition):normalize(-uViewPosition);vec2 r=vec2(0);float n=0.;vec3 U=d;for(int i=0;i<100;i++){U=d+y*n;r=s(U,vInverseWorldMatrix,uBoundsScale);n+=r.x;if(!e(o(U,vInverseWorldMatrix,uBoundsScale),uBoundsScale)||r.x<=1e-4)break;}if(r.x>1e-4)discard;float i=texelFetch(uDepthTexture,ivec2(gl_FragCoord.xy),0).x,P=v(i,uNearClip,uFarClip);vec4 D=uViewMatrix*vec4(U,1);float G=e(D.z,uNearClip,uFarClip);if(G>=P)discard;vec4 C=uProjectionMatrix*uViewMatrix*vec4(U,1);gl_FragDepth=C.z/C.w*.5+.5;if(r.x>0.)l=u(U,vInverseWorldMatrix,uBoundsScale);m=x;
#ifdef USE_ALPHA_TEST
p(m.w,uAlphaTestThreshold);
#endif
m.xyz=pow(m.xyz,vec3(2.2));outGBufferA=vec4(m.xyz,1);outGBufferB=f(l,uShadingModelId);outGBufferC=vec4(uMetallic,uRoughness,0,1);outGBufferD=vec4(uEmissiveColor.xyz.xyz,1);}`;