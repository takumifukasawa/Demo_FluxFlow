...
#endif
vec3 p=vWorldPosition,x=p,z=uIsPerspective>.5?normalize(p-uViewPosition):normalize(-uViewPosition);vec2 U=vec2(0);float e=0.;vec3 i=x;float S=EPS;mat4 P=vInverseWorldMatrix;for(int D=0;D<OI;D++){i=x+z*e;U=o(i,P,uBoundsScale,uUseWorld);e+=U.x;if(!n(v(i,P,uBoundsScale),uBoundsScale))break;if(U.x<=S)break;}if(U.x>S)discard;float D=texelFetch(uDepthTexture,ivec2(gl_FragCoord.xy),0).x,a=d(D,uNearClip,uFarClip);vec4 G=uViewMatrix*vec4(i,1);float O=n(G.z,uNearClip,uFarClip);if(O>=a)discard;vec4 c=uProjectionMatrix*uViewMatrix*vec4(i,1);gl_FragDepth=c.z/c.w*.5+.5;if(U.x>0.)l=u(i,P,uBoundsScale,uUseWorld);f=y;
#ifdef USE_ALPHA_TEST
