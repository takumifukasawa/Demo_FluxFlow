...
#endif
vec3 d=vWorldPosition,y=uIsPerspective>.5?normalize(vWorldPosition-uViewPosition):normalize(-uViewPosition);vec2 r=vec2(0);float l=0.;vec3 U=d;for(int D=0;D<100;D++){U=d+y*l;r=f(U,vInverseWorldMatrix,uBoundsScale);l+=r.x;if(!f(v(U,vInverseWorldMatrix,uBoundsScale),uBoundsScale)||r.x<=1e-4)break;}if(r.x>1e-4)discard;float D=texelFetch(uDepthTexture,ivec2(gl_FragCoord.xy),0).x,P=s(D,uNearClip,uFarClip);vec4 G=uViewMatrix*vec4(U,1);float C=x(G.z,uNearClip,uFarClip);if(C>=P)discard;vec4 e=uProjectionMatrix*uViewMatrix*vec4(U,1);gl_FragDepth=e.z/e.w*.5+.5;if(r.x>0.)z=n(U,vInverseWorldMatrix,uBoundsScale);u=o;
#ifdef USE_ALPHA_TEST
