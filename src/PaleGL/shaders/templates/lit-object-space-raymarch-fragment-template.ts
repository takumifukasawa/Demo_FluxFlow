...
#endif
vec3 l=vWorldPosition,x=l,e=uIsPerspective>.5?normalize(l-uViewPosition):normalize(-uViewPosition);vec2 U=vec2(0);float i=0.;vec3 p=x;float S=EPS;mat4 P=vInverseWorldMatrix;for(int D=0;D<OI;D++){p=x+e*i;U=o(p,P,uBoundsScale,uUseWorld);i+=U.x;if(!n(v(p,P,uBoundsScale),uBoundsScale))break;if(U.x<=S)break;}if(U.x>S)discard;float D=texelFetch(uDepthTexture,ivec2(gl_FragCoord.xy),0).x,a=d(D,uNearClip,uFarClip);vec4 G=uViewMatrix*vec4(p,1);float O=n(G.z,uNearClip,uFarClip);if(O>=a)discard;vec4 c=uProjectionMatrix*uViewMatrix*vec4(p,1);gl_FragDepth=c.z/c.w*.5+.5;if(U.x>0.)z=u(p,P,uBoundsScale,uUseWorld);f=y;
#ifdef USE_ALPHA_TEST
d(f.w,uAlphaTestThreshold);
#endif
f.xyz=pow(f.xyz,vec3(2.2));outGBufferA=vec4(f.xyz,1);outGBufferB=s(z,uShadingModelId);outGBufferC=vec4(uMetallic,uRoughness,0,1);outGBufferD=vec4(r.xyz,1);}`;