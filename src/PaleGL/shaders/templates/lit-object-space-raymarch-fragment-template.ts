...
vec3 v(vec3 v,mat4 u,vec3 d){return(u*vec4(v,1)).xyz*d;}vec2 f(vec3 u,mat4 f,vec3 d){return dfScene(v(u,f,d));}vec3 n(vec3 v,mat4 u,vec3 d){vec3 m=vec3(f(v+vec3(1e-4,0,0),u,d).x-f(v+vec3(-1e-4,0,0),u,d).x,f(v+vec3(0,1e-4,0),u,d).x-f(v+vec3(0,-1e-4,0),u,d).x,f(v+vec3(0,0,1e-4),u,d).x-f(v+vec3(0,0,-1e-4),u,d).x);return normalize(m);}bool f(vec3 v,vec3 u){return abs(v.x)<u.x*.5+1e-4&&abs(v.y)<u.y*.5+1e-4&&abs(v.z)<u.z*.5+1e-4;}float d(float v,float u,float d){return(v+u)/(u-d);}float s(float v,float u,float d){float f=u*v;return-f/(d*(v-1.)-f);}void d(float v,float u){if(v<u)discard;}uniform vec4 uDiffuseColor;uniform sampler2D uDiffuseMap;uniform vec2 uDiffuseMapUvScale;uniform float uSpecularAmount,uAmbientAmount,uMetallic,uRoughness;uniform vec4 uEmissiveColor;uniform int uShadingModelId;
...
#endif
vec3 e=vWorldPosition,z=uIsPerspective>.5?normalize(vWorldPosition-uViewPosition):normalize(-uViewPosition);vec2 r=vec2(0);float l=0.;vec3 U=e;for(int D=0;D<100;D++){U=e+z*l;r=f(U,vInverseWorldMatrix,uBoundsScale);l+=r.x;if(!f(v(U,vInverseWorldMatrix,uBoundsScale),uBoundsScale)||r.x<=1e-4)break;}if(r.x>1e-4)discard;float D=texelFetch(uDepthTexture,ivec2(gl_FragCoord.xy),0).x,y=s(D,uNearClip,uFarClip);vec4 G=uViewMatrix*vec4(U,1);float S=d(G.z,uNearClip,uFarClip);if(S>=y)discard;vec4 a=uProjectionMatrix*uViewMatrix*vec4(U,1);gl_FragDepth=a.z/a.w*.5+.5;if(r.x>0.)x=n(U,vInverseWorldMatrix,uBoundsScale);u=o;
#ifdef USE_ALPHA_TEST
