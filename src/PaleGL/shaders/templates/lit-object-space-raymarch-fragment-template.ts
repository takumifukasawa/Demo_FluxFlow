...
#endif
layout(location=0) out vec4 outGBufferA;layout(location=1) out vec4 outGBufferB;layout(location=2) out vec4 outGBufferC;layout(location=3) out vec4 outGBufferD;void main(){vec4 f=vec4(0,0,0,1);vec2 m=vUv*uDiffuseMapUvScale;vec4 z=uDiffuseColor*texture(uDiffuseMap,m);vec3 i=vNormal;
#ifdef USE_NORMAL_MAP
...
#endif
vec3 x=vWorldPosition,y=uIsPerspective>.5?normalize(vWorldPosition-uViewPosition):normalize(-uViewPosition);vec2 r=vec2(0);float e=0.;vec3 p=x;for(int U=0;U<80;U++){p=x+y*e;r=o(p,vInverseWorldMatrix,uBoundsScale,uUseWorld);e+=r.x;if(!n(v(p,vInverseWorldMatrix,uBoundsScale),uBoundsScale)||r.x<=1e-4)break;}if(r.x>1e-4)discard;float U=texelFetch(uDepthTexture,ivec2(gl_FragCoord.xy),0).x,P=d(U,uNearClip,uFarClip);vec4 D=uViewMatrix*vec4(p,1);float G=n(D.z,uNearClip,uFarClip);if(G>=P)discard;vec4 a=uProjectionMatrix*uViewMatrix*vec4(p,1);gl_FragDepth=a.z/a.w*.5+.5;if(r.x>0.)i=u(p,vInverseWorldMatrix,uBoundsScale,uUseWorld);f=z;
#ifdef USE_ALPHA_TEST
