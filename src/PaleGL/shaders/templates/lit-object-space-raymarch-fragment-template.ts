...
#endif
layout(location=0) out vec4 outGBufferA;layout(location=1) out vec4 outGBufferB;layout(location=2) out vec4 outGBufferC;layout(location=3) out vec4 outGBufferD;void main(){vec4 D=vec4(0,0,0,1);vec2 z=vUv*uDiffuseMapUvScale;vec4 x=uDiffuseColor*texture(uDiffuseMap,z);vec3 m=vNormal;
#ifdef USE_NORMAL_MAP
...
x*=vVertexColor;
#endif
x=vec4(1);vec3 n=vWorldPosition,y=uIsPerspective>.5?normalize(vWorldPosition-uViewPosition):normalize(-uViewPosition);vec2 r=vec2(0);float i=0.;vec3 U=n;for(int l=0;l<100;l++){U=n+y*i;r=s(U,vInverseWorldMatrix,uBoundsScale);i+=r.x;if(!e(o(U,vInverseWorldMatrix,uBoundsScale),uBoundsScale)||r.x<=1e-4)break;}if(r.x>1e-4)discard;float l=texelFetch(uDepthTexture,ivec2(gl_FragCoord.xy),0).x,P=v(l,uNearClip,uFarClip);vec4 G=uViewMatrix*vec4(U,1);float w=e(G.z,uNearClip,uFarClip);if(w>=P)discard;vec4 t=uProjectionMatrix*uViewMatrix*vec4(U,1);gl_FragDepth=t.z/t.w*.5+.5;if(r.x>0.)m=u(U,vInverseWorldMatrix,uBoundsScale);D=x;
#ifdef USE_ALPHA_TEST
f(D.w,uAlphaTestThreshold);
#endif
D.xyz=pow(D.xyz,vec3(2.2));outGBufferA=vec4(D.xyz,1);outGBufferB=d(m,uShadingModelId);outGBufferC=vec4(uMetallic,uRoughness,0,1);outGBufferD=vec4(uEmissiveColor.xyz.xyz,1);}`;