...
#define PI 3.14
#define PI2 6.28
vec2 u(vec2 u,vec2 v){return u.x<v.x?u:v;}mat2 u(float v){float u=sin(v),f=cos(v);return mat2(f,u,-u,f);}vec3 opRe(vec3 v,float u){return v-u*round(v/u);}vec2 opRo(vec2 v,float o){return v*u(-o);}vec3 opTr(vec3 u,vec3 v){return u-v;}vec3 o(vec3 u,vec3 v){return u/v;}float v(float u,vec3 v){return u*min(v.x,min(v.y,v.z));}vec2 o(vec2 v){float u=PI/10.-atan(v.x,v.y),f=PI*2./10.;u=floor(u/f)*f;return opRo(v,-u);}float opSm(float u,float v,float x){float f=clamp(.5+.5*(v-u)/x,0.,1.);return mix(v,u,f)-x*f*(1.-f);}float dfSp(vec3 v,float u){return length(v)-u;}float dfRb(vec3 v,vec3 u,float o){vec3 f=abs(v)-u;return length(max(f,0.))+min(max(f.x,max(f.y,f.z)),0.)-o;}float dfBo(vec3 v,vec3 u){vec3 f=abs(v)-u;return length(max(f,0.))+min(max(f.x,max(f.y,f.z)),0.);}float dfTo(vec3 v,vec2 u){return length(vec2(length(v.xz)-u.x,v.y))-u.y;}float o(vec3 v,float u,float o){vec2 f=vec2(length(v.xz)-2.*u+.05,abs(v.y)-o);return min(max(f.x,f.y),0.)+length(max(f,0.))-.05;}float opWi(vec3 u,vec3 f,float x,vec2 m){u=opTr(u,vec3(m.xy,0));u.xy=opRo(u.xy,x);u.yz=opRo(u.yz,PI*.5);u=o(u,f);float z=o(u,1.,.1);return v(z,f);}vec2 opBu(vec3 v,float u){v/=1.4;vec3 f=v;f.yz=opRo(f.yz,-PI*.5);vec2 o=vec2(10,.6);f.x=abs(f.x);f.xz=opRo(f.xz,PI*sin(sin(uTimelineTime*o.x+u)*cos(uTimelineTime*o.y+u))*.3);float m=opWi(f,vec3(.4,.3,.24)*.2,PI*-.3,vec2(.5,.4)*.2),z=opWi(f,vec3(.32,.3,.2)*.2,PI*.3,vec2(.4,-.4)*.2);return vec2(min(m,z),0);}vec2 opFl(vec3 f,float m){vec2 x=vec2(1e4,-1e4);f/=1.;f.y-=-.8;float r=.1;r=sin(uTimelineTime*1.4+.2+m)*.5;float z=-.1;z=cos(uTimelineTime*1.6+.1+m)*-.5;float l=f.y*sin(f.y*r),y=f.y*sin(f.y*z),s=sin(1.12*r)*.28,P=sin(1.12*z)*.28;mat2 d=u(-1.12*r),n=u(1.12*z);vec3 i=f;i=opTr(i,vec3(l,.28,y));float U=o(i,.015,.28);x=u(x,vec2(U,1));vec3 S=f;S=opTr(S,vec3(s,.56,P));S.yz=opRo(S.yz,PI*.5);S.xz*=d;S.yz*=n;S.xy=o(S.xy);S=opTr(S,vec3(0,.2,sin(S.y*5.)*.105));S.yz=opRo(S.yz,PI*.5);vec3 C=vec3(.08,.2,.2);S=o(S,C);float D=o(S,.4,.01);D=v(D,C);x=u(x,vec2(D,2));vec3 G=f;G=opTr(G,vec3(s,.59,P));float e=dfSp(G,.04);return u(x,vec2(e,3));}float opDb(float u,float v,float f,float m,float x,float o){return mix(mix(mix(v,f,smoothstep(0.,.25,u)),mix(f,m,smoothstep(.25,.5,u)),smoothstep(.25,.5,u)),mix(mix(m,x,smoothstep(.5,.75,u)),mix(x,o,smoothstep(.75,1.,u)),smoothstep(.5,.75,u)),smoothstep(.5,1.,u));}float opTb(float u,float v,float f,float x){return mix(mix(v,f,smoothstep(0.,.5,u)),mix(f,x,smoothstep(.5,1.,u)),smoothstep(.5,1.,u));}
#define BN 16
#define FS 1.
#define CS.35
#define MS.25
uniform vec3 uCP,uBPs[BN],uGPs[4];uniform float uGS,uGM;float v(vec3 v){return sin(v.x*4.+uTimelineTime*3.4)*.07+cos(v.y*3.+uTimelineTime*3.2)*.07+sin(v.z*3.5+uTimelineTime*3.)*.07;}float diMAt(vec3 v){return 1.-smoothstep(1.,1.8,length(v-uCP));}float dfMB(vec3 u,float f){for(int o=0;o<BN;o++){float x=dfSp(opTr(u,uBPs[o].xyz),CS);f=opSm(f,x,MS);}f+=v(u)*diMAt(u);return f;}vec2 opMo(vec2 v,vec2 u,float f){return mix(v,u,f);}vec2 u(vec3 v,float u,float f){vec3 o=mod(v*f,2.)-1.;f*=3.;vec3 m=abs(1.-3.*abs(o));float z=(min(max(m.x,m.y),min(max(m.y,m.z),max(m.z,m.x)))-1.)/f;return vec2(max(u,z),f);}float dfMe(vec3 v){v/=.5;float f=dfBo(v,vec3(1));vec2 m=vec2(f,1);m=u(v,m.x,m.y);m=u(v,m.x,m.y);m=u(v,m.x,m.y);return m.x;}
#pragma RAYMARCH_SCENE
vec3 v(vec3 v,mat4 u,vec3 f){return(u*vec4(v,1)).xyz*f;}vec2 f(vec3 u,mat4 f,vec3 m){return dfScene(v(u,f,m));}vec3 n(vec3 v,mat4 u,vec3 m){vec3 o=vec3(f(v+vec3(1e-4,0,0),u,m).x-f(v+vec3(-1e-4,0,0),u,m).x,f(v+vec3(0,1e-4,0),u,m).x-f(v+vec3(0,-1e-4,0),u,m).x,f(v+vec3(0,0,1e-4),u,m).x-f(v+vec3(0,0,-1e-4),u,m).x);return normalize(o);}bool f(vec3 v,vec3 u){return abs(v.x)<u.x*.5+1e-4&&abs(v.y)<u.y*.5+1e-4&&abs(v.z)<u.z*.5+1e-4;}float x(float v,float u,float f){return(v+u)/(u-f);}float s(float v,float u,float f){float o=u*v;return-o/(f*(v-1.)-o);}void n(float v,float u){if(v<u)discard;}uniform vec4 uDiffuseColor;uniform sampler2D uDiffuseMap;uniform vec2 uDiffuseMapUvScale;uniform float uSpecularAmount,uAmbientAmount,uMetallic,uRoughness;uniform vec4 uEmissiveColor;uniform int uShadingModelId;
...
o*=vVertexColor;
#endif
vec3 S=uEmissiveColor.xyz;
#ifdef USE_INSTANCING
S=vInstanceEmissiveColor.xyz;
#endif
vec3 d=vWorldPosition,y=uIsPerspective>.5?normalize(vWorldPosition-uViewPosition):normalize(-uViewPosition);vec2 r=vec2(0);float l=0.;vec3 U=d;for(int i=0;i<100;i++){U=d+y*l;r=f(U,vInverseWorldMatrix,uBoundsScale);l+=r.x;if(!f(v(U,vInverseWorldMatrix,uBoundsScale),uBoundsScale)||r.x<=1e-4)break;}if(r.x>1e-4)discard;float i=texelFetch(uDepthTexture,ivec2(gl_FragCoord.xy),0).x,P=s(i,uNearClip,uFarClip);vec4 D=uViewMatrix*vec4(U,1);float G=x(D.z,uNearClip,uFarClip);if(G>=P)discard;vec4 C=uProjectionMatrix*uViewMatrix*vec4(U,1);gl_FragDepth=C.z/C.w*.5+.5;if(r.x>0.)z=n(U,vInverseWorldMatrix,uBoundsScale);u=o;
#ifdef USE_ALPHA_TEST
n(u.w,uAlphaTestThreshold);
#endif
u.xyz=pow(u.xyz,vec3(2.2));outGBufferA=vec4(u.xyz,1);outGBufferB=s(z,uShadingModelId);outGBufferC=vec4(uMetallic,uRoughness,0,1);outGBufferD=vec4(S.xyz,1);}`;