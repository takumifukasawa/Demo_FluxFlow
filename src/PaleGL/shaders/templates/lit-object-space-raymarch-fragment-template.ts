...
#endif
layout(location=0) out vec4 outGBufferA;layout(location=1) out vec4 outGBufferB;layout(location=2) out vec4 outGBufferC;layout(location=3) out vec4 outGBufferD;void main(){vec4 m=vec4(0,0,0,1);vec2 z=vUv*uDiffuseMapUvScale;vec4 d=uDiffuseColor*texture(uDiffuseMap,z);vec3 i=vNormal;
#ifdef USE_NORMAL_MAP
...
d*=vVertexColor;
#endif
d=vec4(1);vec3 n=vWorldPosition,y=uIsPerspective>.5?normalize(vWorldPosition-uViewPosition):normalize(-uViewPosition);vec2 r=vec2(0);float x=0.;vec3 l=n;for(int U=0;U<100;U++){l=n+y*x;r=s(l,vInverseWorldMatrix,uBoundsScale);x+=r.x;if(!e(o(l,vInverseWorldMatrix,uBoundsScale),uBoundsScale)||r.x<=1e-4)break;}if(r.x>1e-4)discard;float U=texelFetch(uDepthTexture,ivec2(gl_FragCoord.xy),0).x,P=v(U,uNearClip,uFarClip);vec4 D=uViewMatrix*vec4(l,1);float G=e(D.z,uNearClip,uFarClip);if(G>=P)discard;vec4 C=uProjectionMatrix*uViewMatrix*vec4(l,1);gl_FragDepth=C.z/C.w*.5+.5;if(r.x>0.)i=u(l,vInverseWorldMatrix,uBoundsScale);m=d;
#ifdef USE_ALPHA_TEST
p(m.w,uAlphaTestThreshold);
#endif
m.xyz=pow(m.xyz,vec3(2.2));outGBufferA=vec4(m.xyz,1);outGBufferB=f(i,uShadingModelId);outGBufferC=vec4(uMetallic,uRoughness,0,1);outGBufferD=vec4(uEmissiveColor.xyz.xyz,1);}`;