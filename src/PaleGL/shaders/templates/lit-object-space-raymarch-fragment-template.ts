...
#endif
vec3 z=vWorldPosition,e=uIsPerspective>.5?normalize(vWorldPosition-uViewPosition):normalize(-uViewPosition);vec2 r=vec2(0);float l=0.;vec3 U=z;for(int D=0;D<100;D++){U=z+e*l;r=n(U,vInverseWorldMatrix,uBoundsScale);l+=r.x;if(!d(v(U,vInverseWorldMatrix,uBoundsScale),uBoundsScale)||r.x<=1e-4)break;}if(r.x>1e-4)discard;float D=texelFetch(uDepthTexture,ivec2(gl_FragCoord.xy),0).x,y=x(D,uNearClip,uFarClip);vec4 G=uViewMatrix*vec4(U,1);float S=s(G.z,uNearClip,uFarClip);if(S>=y)discard;vec4 a=uProjectionMatrix*uViewMatrix*vec4(U,1);gl_FragDepth=a.z/a.w*.5+.5;if(r.x>0.)m=d(U,vInverseWorldMatrix,uBoundsScale);f=o;
#ifdef USE_ALPHA_TEST
